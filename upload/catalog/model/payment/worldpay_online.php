...
class ModelPaymentWorldpayOnline extends Model {
	...
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('worldpay_online_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('worldpay_online_total') > 0 && $this->config->get('worldpay_online_total') > $total) {
			$status = false;
		} elseif ($this->config->has('worldpay_online_total_max') && $this->config->get('worldpay_online_total_max') > 0 && $total > $this->config->get('worldpay_online_total_max')) {
			$status = false;
		} elseif (!$this->config->get('worldpay_online_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				'code'       => 'worldpay_online',
				'title'      => $this->language->get('text_title'),
				'terms'      => '',
				'sort_order' => $this->config->get('worldpay_online_sort_order')
			);
		}
	...
	public function deleteCard($token) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "worldpay_online_card WHERE customer_id = '" . $this->customer->isLogged() . "' AND token = '" . $this->db->escape($token) . "'");
	...
	public function getOrder($order_id) {
		...
		if ($query->num_rows) {
			...
			$order['transactions'] = $this->getTransactions($order['worldpay_online_order_id']);
	...
	public function cronPayment() {
		...
		$log = new Log('worldpay_online_recurring_orders.log');
	...
	private function getProfiles() {
		$sql = "SELECT orr.order_recurring_id FROM " . DB_PREFIX . "order_recurring orr JOIN `" . DB_PREFIX . "order` o USING(`order_id`) WHERE o.payment_code = 'worldpay_online'";
	...
	public function updateCronJobRunTime() {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'worldpay_online' AND `key` = 'worldpay_online_last_cron_job_run'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'worldpay_online', 'worldpay_online_last_cron_job_run', NOW(), 0)");
	}
	...
	public function logger($data) {
		if ($this->config->get('worldpay_online_debug')) {
			$log = new Log('worldpay_online_debug.log');
	...
	public function recurringPayments() {
		// Used by the checkout to state the module supports recurring profiles.
		return true;
	}
}
