	...
	public function getMethod($address, $total) {
		...
		if ($status) {
			...
			if ($klarna_invoice[$address['iso_code_3']]['total'] > 0 && $klarna_invoice[$address['iso_code_3']]['total'] > $total) {
				$status = false;
			} elseif (isset($klarna_invoice[$address['iso_code_3']]['total_max']) && $klarna_invoice[$address['iso_code_3']]['total_max'] > 0 && $total > $klarna_invoice[$address['iso_code_3']]['total_max']) {
				$status = false;
