...
class ModelPaymentGlobalpayRemote extends Model {
	...
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('globalpay_remote_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('globalpay_remote_total') > 0 && $this->config->get('globalpay_remote_total') > $total) {
			$status = false;
		} elseif ($this->config->has('globalpay_remote_total_max') && $this->config->get('globalpay_remote_total_max') > 0 && $total > $this->config->get('globalpay_remote_total_max')) {
			$status = false;
	...
	public function capturePayment($account, $amount, $currency, $order_id, $order_ref, $card_number, $expire, $name, $type, $cvv, $issue, $eci_ref, $eci = '', $cavv = '', $xid = '') {
		...
		if ($response->result == '00') {
			...
			if ($this->config->get('globalpay_remote_auto_settle') == 1) {
				...
				$this->model_checkout_order->update($order_id, $this->config->get('globalpay_remote_order_status_success_settled_id'), $message);
			} else {
				$this->addTransaction($globalpay_order_id, 'auth', 0);
				$this->model_checkout_order->update($order_id, $this->config->get('globalpay_remote_order_status_success_unsettled_id'), $message);
			}
