...
class ModelPaymentSagePayServer extends Model {
	...
	public function getMethod($address, $total) {
		...
		if ($this->config->get('sagepay_server_total') > 0 && $this->config->get('sagepay_server_total') > $total) {
			$status = false;
		} elseif ($this->config->has('sagepay_server_total_max') && $this->config->get('sagepay_server_total_max') > 0 && $total > $this->config->get('sagepay_server_total_max')) {
			$status = false;
	...
	public function addRecurringPayment($item, $vendor_tx_code) {
		...
		$recurring_amt = $this->currency->format($this->tax->calculate($item['recurring_price'], $item['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false) * $item['quantity'] . ' ' . $this->currency->getCode();
	...
	private function getProfiles() {
		$sql = "SELECT orr.order_recurring_id FROM " . DB_PREFIX . "order_recurring orr JOIN `" . DB_PREFIX . "order` o USING(`order_id`) WHERE o.payment_code = 'sagepay_server'";
