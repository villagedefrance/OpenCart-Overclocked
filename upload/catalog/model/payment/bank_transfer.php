...
class ModelPaymentBankTransfer extends Model {
	...
	public function getMethod($address, $total) {
		...
		if (($this->config->get('bank_transfer_total') > 0 && $this->config->get('bank_transfer_total') > $total) || ($this->config->get('bank_transfer_total_max') > 0 && $this->config->get('bank_transfer_total_max') < $total)) {
			$status = false;
