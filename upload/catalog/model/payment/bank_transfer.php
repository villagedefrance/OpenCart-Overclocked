...
class ModelPaymentBankTransfer extends Model {
	...
	public function getMethod($address, $total) {
		...
		$total_max = $this->config->get('bank_transfer_total_max');

		if (($this->config->get('bank_transfer_total') > 0 && $this->config->get('bank_transfer_total') > $total) || (!empty($total_max) && $total_max > 0 && $total_max < $total)) {
			$status = false;
