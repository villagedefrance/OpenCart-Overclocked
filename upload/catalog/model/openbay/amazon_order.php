...
class ModelOpenbayAmazonOrder extends Model {
  ...
  <MerchantOrderId>$orderId</MerchantOrderId>
	...
	public function addAmazonOrder($orderId, $amazonOrderId) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "amazon_order` (`order_id`, `amazon_order_id`) VALUES (" . (int)$orderId . ", '" . $this->db->escape($amazonOrderId) . "')");
	}
	...
	public function addAmazonOrderProducts($orderId, $data) {
		foreach ($data as $sku => $orderItemId) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "amazon_order_product` (`order_product_id`, `amazon_order_item_id`) VALUES (" . (int)$orderProductId . ", '" . $this->db->escape($orderItemId) . "')");
		}
	}
	...
	public function getProductOptionsByVar($productVar) {
		...
		foreach ($optionValueIds as $optionValueId) {
			...
			FROM " . DB_PREFIX . "product_option_value pov, " . DB_PREFIX . "product_option po, `" . DB_PREFIX . "option` opt, " . DB_PREFIX . "option_value_description ovd, " . DB_PREFIX . "option_description od 
