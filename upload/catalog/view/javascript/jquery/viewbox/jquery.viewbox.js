...
(function($) {$.fn.viewbox = function(options) {
	...
	function show($e) {
		...
		var href = $e.attr('href'), caption = options.setTitle && $e.attr('title') ? $e.attr('title') : '';
	...
	function openWindow(width,height) {
		var $body = get('body'), $content = get('content'), $header = get('header'), w, h;
	...
	function showImage(href,caption) {
		...
		var $body = get('body'),
			...
			timerId = window.setInterval(function() {
				...
				$content.animate({width: w, height: h}, options.resizeDuration, function() {
					$content.append($img);
					$header.show().width(w);
					set('header', caption);
					locked = false;
