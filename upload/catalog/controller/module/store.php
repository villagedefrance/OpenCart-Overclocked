...
class ControllerModuleStore extends Controller { 
	...
	protected function index($setting) { 
		...
		if ($status) { 
			...
			if ($this->user) { $this->data['username'] = $this->user->getUserName(); } else { $this->data['username'] = ''; } 
			if ($this->user) { $this->data['userid'] = $this->user->getId(); } else { $this->data['userid'] = ''; } 
			...
			if (isset($setting['adlink'])) { $this->data['adminlink'] = 1; } else { $this->data['adminlink'] = 0; } 
