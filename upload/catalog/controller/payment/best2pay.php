...
class ControllerPaymentBest2pay extends Controller {
	...
	public function index() {
		...
		$currency_code = $this->config->get('best2pay_currency');

		$total = $this->currency->convert($order_info['total'], $order_info['currency_code'], $currency_code);

		$commission = ($this->config->get('best2pay_commission_pay') == 1 && $this->config->get('best2pay_commission') && floatval($this->config->get('best2pay_commission'))) ? floatval($this->config->get('best2pay_commission')) : 0;
		...
		$register_data['sector'] = $this->config->get('best2pay_sector');
		$register_data['url'] = HTTP_SERVER . 'index.php?route=payment/best2pay/request';
		$register_data['email'] = $order_info['email'];
		$register_data['phone'] = $order_info['telephone'];
		$register_data['mode'] = 1;
		$register_data['signature'] = base64_encode(md5($register_data['sector'] . $register_data['amount'] . $register_data['currency'] . $this->config->get('best2pay_password')));
		...
		if ($order_id) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/best2pay.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/payment/best2pay.tpl';
			} else {
				$this->template = 'default/template/payment/best2pay.tpl';
			}
	...
	public function callback() {
		...
		if (isset($result->reason_code) && $result->reason_code == 1) {
			...
			$this->model_checkout_order->confirm($result->reference, $this->config->get('best2pay_order_status_id'));

			$message = 'Best2pay callback.';

			$this->model_checkout_order->update($result->reference, $this->config->get('best2pay_order_status_id'), $message, false);
		}
	}
