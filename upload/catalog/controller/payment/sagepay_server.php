...
class ControllerPaymentSagepayServer extends Controller {
	...
	public function index() {
		...
		$this->data['action'] = $this->url->link('payment/sagepay_server/send', '', 'SSL');

		if ($this->config->get('sagepay_server_card') == '1') {
			$this->data['sagepay_server_card'] = true;
		} else {
		...
		if ($this->customer->isLogged() && $this->data['sagepay_server_card']) {
			...
			$this->data['cards'] = $this->model_payment_sagepay_server->getCards($this->customer->getId());
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/sagepay_server.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/sagepay_server.tpl';
		} else {
			$this->template = 'default/template/payment/sagepay_server.tpl';
		}
	...
	public function callback() {
		...
		$this->model_checkout_order->update($order_id, $this->config->get('sagepay_server_order_status_id'), $comment);
		...
		if (!empty($str_token)) {
			...
			$this->data['ExpiryDate'] = substr($str_expiry_date, -4, 2) . '/' . substr($str_expiry_date, 2);
			$this->data['Token'] = $str_token;
			$this->data['CardType'] = $str_card_type;
			$this->data['Last4Digits'] = $str_last_4_digits;

			$this->model_payment_sagepay_server->addCard($this->data);
		}
	...
	public function delete() {
		$this->language->load('account/sagepay_server_cards');
