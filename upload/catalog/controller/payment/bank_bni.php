...
class ControllerPaymentBankBni extends Controller {
	...
	protected function index() {
		...
		$this->data['bank'] = nl2br($this->config->get('bank_bni_bank_' . $this->config->get('config_language_id')));
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/bank_bni.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/bank_bni.tpl';
		} else {
			$this->template = 'default/template/payment/bank_bni.tpl';
		}
