...
class ControllerPaymentSkrill extends Controller {
	...
	public function index() {
		...
		$this->data['return_url'] = $this->url->link('checkout/success', '', 'SSL');
		$this->data['cancel_url'] = $this->url->link('checkout/checkout', '', 'SSL');
		$this->data['status_url'] = $this->url->link('payment/skrill/callback', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/skrill.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/skrill.tpl';
		} else {
			$this->template = 'default/template/payment/skrill.tpl';
		}
	...
	public function callback() {
		...
		if ($order_info) {
			...
			if ($this->config->get('skrill_secret')) {
				...
				if (($md5hash != $md5sig) || (strtolower($this->request->post['pay_to_email']) != strtolower($this->config->get('skrill_email'))) || ((float)$this->request->post['amount'] != $this->currency->format((float)$order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false))) {
					$verified = false;
				}
			}
			...
			if ($verified) {
				switch ($this->request->post['status']) {
					case '2':
						...
						break;
					case '0':
						$this->model_checkout_order->update($order_id, $this->config->get('skrill_pending_status_id'), '', true);
						break;
					case '-1':
						$this->model_checkout_order->update($order_id, $this->config->get('skrill_canceled_status_id'), '', true);
						break;
					case '-2':
						$this->model_checkout_order->update($order_id, $this->config->get('skrill_failed_status_id'), '', true);
						break;
					case '-3':
						$this->model_checkout_order->update($order_id, $this->config->get('skrill_chargeback_status_id'), '', true);
						break;
				}
			} else {
