	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$user_group_id = $this->session->data['new_user_group_id'];

				if ($user_group_id) {
					unset($this->session->data['new_user_group_id']);

					$this->redirect($this->url->link('user/user_permission/update', 'token=' . $this->session->data['token'] . '&user_group_id=' . $user_group_id . $url, 'SSL'));
				}
