<?php
class ControllerShippingDigital extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('digital', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			if (isset($this->request->post['apply'])) {
				$this->redirect($this->url->link('shipping/digital', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/digital', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$this->data['action'] = $this->url->link('shipping/digital', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['digital_total'])) {
			$this->data['digital_total'] = $this->request->post['digital_total'];
		} else {
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'shipping/digital')) {
