...
class ControllerDesignCustomField extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$custom_field_id = $this->session->data['new_custom_field_id'];

				if ($custom_field_id) {
					unset($this->session->data['new_custom_field_id']);

					$this->redirect($this->url->link('design/custom_field/update', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $custom_field_id . $url, 'SSL'));
				}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$custom_field_id = $this->request->get['custom_field_id'];
