	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$banner_id = $this->session->data['new_banner_id'];

				if ($banner_id) {
					unset($this->session->data['new_banner_id']);

					$this->redirect($this->url->link('design/banner/update', 'token=' . $this->session->data['token'] . '&banner_id=' . $banner_id . $url, 'SSL'));
				}
