	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$voucher_theme_id = $this->session->data['new_voucher_theme_id'];

				if ($voucher_theme_id) {
					unset($this->session->data['new_voucher_theme_id']);

					$this->redirect($this->url->link('sale/voucher_theme/update', 'token=' . $this->session->data['token'] . '&voucher_theme_id=' . $voucher_theme_id . $url, 'SSL'));
				}
