...
class ControllerSaleCustomerGroup extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$customer_group_id = $this->session->data['new_customer_group_id'];

				if ($customer_group_id) {
					unset($this->session->data['new_customer_group_id']);

					$this->redirect($this->url->link('sale/customer_group/update', 'token=' . $this->session->data['token'] . '&customer_group_id=' . $customer_group_id . $url, 'SSL'));
				}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image'])) {
			...
			$this->data['image'] = $customer_group_info['image'];
		} else {
			$this->data['image'] = '';
		}
		...
		} elseif (!empty($customer_group_info) && $customer_group_info['image'] && file_exists(DIR_IMAGE . $customer_group_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($customer_group_info['image'], 100, 100);
		} else {
