	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$language_id = $this->session->data['new_language_id'];

				if ($language_id) {
					unset($this->session->data['new_language_id']);

					$this->redirect($this->url->link('localisation/language/update', 'token=' . $this->session->data['token'] . '&language_id=' . $language_id . $url, 'SSL'));
				}
