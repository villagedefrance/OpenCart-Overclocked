	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$tax_rate_id = $this->session->data['new_tax_rate_id'];

				if ($tax_rate_id) {
					unset($this->session->data['new_tax_rate_id']);

					$this->redirect($this->url->link('localisation/tax_rate/update', 'token=' . $this->session->data['token'] . '&tax_rate_id=' . $tax_rate_id . $url, 'SSL'));
				}
