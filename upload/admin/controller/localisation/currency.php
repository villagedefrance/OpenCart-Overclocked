	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$currency_id = $this->session->data['new_currency_id'];

				if ($currency_id) {
					unset($this->session->data['new_currency_id']);

					$this->redirect($this->url->link('localisation/currency/update', 'token=' . $this->session->data['token'] . '&currency_id=' . $currency_id . $url, 'SSL'));
				}
