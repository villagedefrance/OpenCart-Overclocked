	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$length_class_id = $this->session->data['new_length_class_id'];

				if ($length_class_id) {
					unset($this->session->data['new_length_class_id']);

					$this->redirect($this->url->link('localisation/length_class/update', 'token=' . $this->session->data['token'] . '&length_class_id=' . $length_class_id . $url, 'SSL'));
				}
