	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$geo_zone_id = $this->session->data['new_geo_zone_id'];

				if ($geo_zone_id) {
					unset($this->session->data['new_geo_zone_id']);

					$this->redirect($this->url->link('localisation/geo_zone/update', 'token=' . $this->session->data['token'] . '&geo_zone_id=' . $geo_zone_id . $url, 'SSL'));
				}
