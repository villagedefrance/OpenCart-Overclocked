...
class ControllerLocalisationReturnStatus extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_status_id = $this->session->data['new_return_status_id'];

				if ($return_status_id) {
					unset($this->session->data['new_return_status_id']);

					$this->redirect($this->url->link('localisation/return_status/update', 'token=' . $this->session->data['token'] . '&return_status_id=' . $return_status_id . $url, 'SSL'));
				}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_status_id = $this->request->get['return_status_id'];
