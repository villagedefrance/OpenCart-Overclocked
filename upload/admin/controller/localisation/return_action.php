...
class ControllerLocalisationReturnAction extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_action_id = $this->session->data['new_return_action_id'];

				if ($return_action_id) {
					unset($this->session->data['new_return_action_id']);

					$this->redirect($this->url->link('localisation/return_action/update', 'token=' . $this->session->data['token'] . '&return_action_id=' . $return_action_id . $url, 'SSL'));
				}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_action_id = $this->request->get['return_action_id'];
