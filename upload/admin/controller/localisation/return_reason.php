...
class ControllerLocalisationReturnReason extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_reason_id = $this->session->data['new_return_reason_id'];

				if ($return_reason_id) {
					unset($this->session->data['new_return_reason_id']);

					$this->redirect($this->url->link('localisation/return_reason/update', 'token=' . $this->session->data['token'] . '&return_reason_id=' . $return_reason_id . $url, 'SSL'));
				}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$return_reason_id = $this->request->get['return_reason_id'];
