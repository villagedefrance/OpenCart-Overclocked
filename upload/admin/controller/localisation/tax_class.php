...
class ControllerLocalisationTaxClass extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$tax_class_id = $this->session->data['new_tax_class_id'];

				if ($tax_class_id) {
					unset($this->session->data['new_tax_class_id']);

					$this->redirect($this->url->link('localisation/tax_class/update', 'token=' . $this->session->data['token'] . '&tax_class_id=' . $tax_class_id . $url, 'SSL'));
				}
	...
	protected function getList() {
		...
		$this->data['button_rates'] = $this->language->get('button_rates');
		$this->data['button_insert'] = $this->language->get('button_insert');
		$this->data['button_delete'] = $this->language->get('button_delete');

		$this->data['tax_rates'] = $this->url->link('localisation/tax_rate', 'token=' . $this->session->data['token'] . $url, 'SSL');
