...
class ControllerCatalogInformation extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$information_id = $this->session->data['new_information_id'];

				if ($information_id) {
					unset($this->session->data['new_information_id']);

					$this->redirect($this->url->link('catalog/information/update', 'token=' . $this->session->data['token'] . '&information_id=' . $information_id . $url, 'SSL'));
				}
	...
	protected function getList() {
		...
		$this->data['disabled'] = $this->url->link('catalog/information/disable', 'token=' . $this->session->data['token'] . $url, 'SSL');
