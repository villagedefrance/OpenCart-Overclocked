	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$attribute_id = $this->session->data['new_attribute_id'];

				if ($attribute_id) {
					unset($this->session->data['new_attribute_id']);

					$this->redirect($this->url->link('catalog/attribute/update', 'token=' . $this->session->data['token'] . '&attribute_id=' . $attribute_id . $url, 'SSL'));
				}
