	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$filter_group_id = $this->session->data['new_filter_group_id'];

				if ($filter_group_id) {
					unset($this->session->data['new_filter_group_id']);

					$this->redirect($this->url->link('catalog/filter/update', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $filter_group_id . $url, 'SSL'));
				}
