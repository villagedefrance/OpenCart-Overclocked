	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$attribute_group_id = $this->session->data['new_attribute_group_id'];

				if ($attribute_group_id) {
					unset($this->session->data['new_attribute_group_id']);

					$this->redirect($this->url->link('catalog/attribute_group/update', 'token=' . $this->session->data['token'] . '&attribute_group_id=' . $attribute_group_id . $url, 'SSL'));
				}
