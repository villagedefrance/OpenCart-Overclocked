	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply'])) {
				$option_id = $this->session->data['new_option_id'];

				if ($option_id) {
					unset($this->session->data['new_option_id']);

					$this->redirect($this->url->link('catalog/option/update', 'token=' . $this->session->data['token'] . '&option_id=' . $option_id . $url, 'SSL'));
				}
