...
class ModelOpenbayAmazon extends Model {
  ...
  <MarketplaceIDs>';
	...
	public function getProductStatus($product_id) {
		...
		$links = $this->db->query("SELECT COUNT(*) as count FROM `" . DB_PREFIX . "amazon_product_link` WHERE `product_id` = '" . (int)$product_id . "'")->row;
	...
	public function getProductErrors($product_id, $version = 2) {
		if ($version == 3) {
			$messageRow = $this->db->query("SELECT `messages` FROM `" . DB_PREFIX . "amazon_product` WHERE `product_id` = '" . (int)$product_id . "' AND `version` = 3")->row;
		...
		$insertionRows = $this->db->query("SELECT `sku`, `insertion_id` FROM `" . DB_PREFIX . "amazon_product` WHERE `product_id` = '" . (int)$product_id . "' AND `version` = 2")->rows;
