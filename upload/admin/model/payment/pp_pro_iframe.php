...
class ModelPaymentPPProIframe extends Model {
	...
	protected function getTransactions($paypal_iframe_order_id) {
		$query = $this->db->query("SELECT ot.*, (SELECT COUNT(ot2.paypal_iframe_order_id) FROM " . DB_PREFIX . "paypal_iframe_order_transaction ot2 WHERE ot2.parent_transaction_id = ot.transaction_id) AS children FROM " . DB_PREFIX . "paypal_iframe_order_transaction ot WHERE paypal_iframe_order_id = '" . (int)$paypal_iframe_order_id . "'"); 
	...
	public function getOrder($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "paypal_iframe_order WHERE order_id = '" . (int)$order_id . "' LIMIT 0,1");
	...
	public function addTransaction($transaction_data, $request_data = array()) {
		...
		if ($request_data) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "paypal_iframe_order_transaction SET call_data = '" . $this->db->escape($serialized_data) . "' WHERE paypal_iframe_order_transaction_id = " . (int)$paypal_iframe_order_transaction_id . " LIMIT 0,1");
		}
	...
	public function getOrderId($transaction_id) {
		$query = $this->db->query("SELECT o.order_id AS order_id FROM " . DB_PREFIX . "paypal_iframe_order_transaction ot LEFT JOIN " . DB_PREFIX . "paypal_iframe_order o ON (o.paypal_iframe_order_id = ot.paypal_iframe_order_id) WHERE ot.transaction_id = '" . $this->db->escape($transaction_id) . "' LIMIT 0,1");
