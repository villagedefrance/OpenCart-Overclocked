...
class ModelPaymentPPExpress extends Model {
	...
	public function addTransaction($transaction_data, $request_data = array()) {
		...
		if ($request_data) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "paypal_order_transaction SET call_data = '" . $this->db->escape($serialized_data) . "' WHERE paypal_order_transaction_id = " . (int)$paypal_order_transaction_id . " LIMIT 0,1");
		}
	...
	public function getFailedTransaction($paypl_order_transaction_id) {
		$result = $this->db->query("SELECT * FROM " . DB_PREFIX . "paypal_order_transaction WHERE paypal_order_transaction_id = " . (int)$paypl_order_transaction_id . "")->row;
	...
	protected function getTransactions($paypal_order_id) {
	...
	public function getOrderId($transaction_id) {
		$query = $this->db->query("SELECT o.order_id AS order_id FROM " . DB_PREFIX . "paypal_order_transaction ot LEFT JOIN " . DB_PREFIX . "paypal_order o ON (o.paypal_order_id = ot.paypal_order_id) WHERE ot.transaction_id = '" . $this->db->escape($transaction_id) . "' LIMIT 0,1");
