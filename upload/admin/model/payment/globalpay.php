...
class ModelPaymentGlobalpay extends Model {
	...
	public function install() {
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "globalpay_order` (
	...
	public function updateForRebate($globalpay_order_id, $pas_ref, $order_ref) {
		$this->db->query("UPDATE " . DB_PREFIX . "globalpay_order SET order_ref_previous = '_multisettle_" . $this->db->escape($order_ref) . "', pasref_previous = '" . $this->db->escape($pas_ref) . "' WHERE globalpay_order_id = '" . (int)$globalpay_order_id . "' LIMIT 0,1");
	}
	...
	public function getOrder($order_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "globalpay_order WHERE order_id = '" . (int)$order_id . "' LIMIT 0,1");
