...
class ModelSaleOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		// Restock products before substracting the stock later on
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_email'])) {
			$sql .= " AND o.email = '" . (int)$data['filter_email'] . "'";
		}
	...
	public function addOrderHistory($order_id, $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$data['order_status_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', `comment` = '" . $this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW()");
		...
		if ($data['notify']) {
			...
			if ($data['tracking']) {
				$message .= $language->get('text_tracking') . ' ' . html_entity_decode($data['tracking'], ENT_QUOTES, 'UTF-8') . "\n\n";
			}
