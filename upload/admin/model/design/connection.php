...
class ModelDesignConnection extends Model {
	...
	public function addConnection($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "connection SET name = '" . $this->db->escape($data['name']) . "'");
	...
	public function editConnection($connection_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "connection SET name = '" . $this->db->escape($data['name']) . "' WHERE connection_id = '" . (int)$connection_id . "'");
