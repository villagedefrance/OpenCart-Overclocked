...
class ModelLocalisationCurrency extends Model {
	...
	public function updateCurrencies() {
		...
		if (extension_loaded('curl')) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "currency WHERE code != '" . $this->db->escape($this->config->get('config_currency')) . "' AND date_modified < '" . $this->db->escape(date('Y-m-d H:i:s', strtotime('-1 day'))) . "' AND status = '1'");
