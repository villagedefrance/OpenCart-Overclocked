...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" name="category">
        <table class="form">
        <tbody>
          <tr>
        ...
        <tbody id="header-1" class="module-header">
          <tr style="background:#FCFCFC;">
            ...
            <td><?php foreach ($languages as $language) { ?>
              <input type="text" name="category_title<?php echo $language['language_id']; ?>" id="category_title<?php echo $language['language_id']; ?>" size="30" value="<?php echo ${'category_title' . $language['language_id']}; ?>" />
          ...
          <tr style="background:#FCFCFC;">
            ...
            <td><select name="category_header_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $category_header_color) { ?>
          ...
          <tr style="background:#FCFCFC;">
            ...
            <td><select name="category_header_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $category_header_shape) { ?>
        ...
        <tbody>
          <tr>
            ...
            <td><select name="category_content_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $category_content_color) { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;" selected="selected"><?php echo $skin['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;"><?php echo $skin['title']; ?></option>
                <?php } ?>
          ...
          <tr>
            ...
            <td><select name="category_content_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $category_content_shape) { ?>
                  <option value="<?php echo $shape['shape']; ?>" selected="selected"><?php echo $shape['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $shape['shape']; ?>"><?php echo $shape['title']; ?></option>
                <?php } ?>
        ...
        <?php foreach ($modules as $module) { ?>
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              ...
              <td class="center">
                <input type="text" name="category_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" size="3" />
              </td>
              <td class="center">
...
$('input[name=\'category_theme\']').bind('change', function() {
	$('.module-header').hide();
	$('#header-' + this.value).show();
});

$('input[name=\'category_theme\']:checked').trigger('change');
//--></script>
...
function addModule() {
	...
	html += '    </select></td>';
	html += '    <td class="center"><input type="text" name="category_module[' + module_row + '][sort_order]" value="" size="3" /></td>';
