...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" name="latest">
        <table class="form">
        <tbody>
          <tr>
        ...
        <tbody id="header-1" class="module-header">
          <tr class="highlighted">
            ...
            <td><?php foreach ($languages as $language) { ?>
              <input type="text" name="latest_title<?php echo $language['language_id']; ?>" id="latest_title<?php echo $language['language_id']; ?>" size="30" value="<?php echo ${'latest_title' . $language['language_id']}; ?>" />
          ...
          <?php echo $stylesheet_mode ? '<tr style="display:none;">' : '<tr class="highlighted">'; ?>
            ...
            <td><select name="latest_header_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $latest_header_color) { ?>
          ...
          <?php echo $stylesheet_mode ? '<tr style="display:none;">' : '<tr class="highlighted">'; ?>
            ...
            <td><select name="latest_header_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $latest_header_shape) { ?>
        ...
        <tbody>
          <?php echo $stylesheet_mode ? '<tr style="display:none;">' : '<tr>'; ?>
            ...
            <td><select name="latest_content_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $latest_content_color) { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;" selected="selected"><?php echo $skin['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;"><?php echo $skin['title']; ?></option>
                <?php } ?>
          ...
          <?php echo $stylesheet_mode ? '<tr style="display:none;">' : '<tr>'; ?>
            ...
            <td><select name="latest_content_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $latest_content_shape) { ?>
                  <option value="<?php echo $shape['shape']; ?>" selected="selected"><?php echo $shape['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $shape['shape']; ?>"><?php echo $shape['title']; ?></option>
                <?php } ?>
        ...
        <?php foreach ($modules as $module) { ?>
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              ...
              <td class="left">
                <input type="text" name="latest_module[<?php echo $module_row; ?>][image_width]" value="<?php echo $module['image_width']; ?>" size="3" /> x 
                <input type="text" name="latest_module[<?php echo $module_row; ?>][image_height]" value="<?php echo $module['image_height']; ?>" size="3" /> px
              ...
              <td class="center">
                <input type="text" name="latest_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" size="3" />
              </td>
              <td class="center">
...
$('input[name=\'latest_theme\']').bind('change', function() {
	$('.module-header').hide();
	$('#header-' + this.value).show();
});

$('input[name=\'latest_theme\']:checked').trigger('change');
//--></script>
...
function addModule() {
	...
	html += '    </select></td>';
	html += '    <td class="center"><input type="text" name="latest_module[' + module_row + '][sort_order]" value="" size="3" /></td>';
