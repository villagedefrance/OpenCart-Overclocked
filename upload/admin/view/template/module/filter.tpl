...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" name="filter">
        <table class="form">
        <tbody>
          <tr>
            <td><?php echo $entry_theme; ?></td>
            <td><?php if ($filter_theme) { ?>
              <input type="radio" name="filter_theme" value="1" id="theme-on" class="radio" checked />
              <label for="theme-on"><span><span></span></span><?php echo $text_yes; ?></label>
              <input type="radio" name="filter_theme" value="0" id="theme-off" class="radio" />
              <label for="theme-off"><span><span></span></span><?php echo $text_no; ?></label>
            <?php } else { ?>
              <input type="radio" name="filter_theme" value="1" id="theme-on" class="radio" />
              <label for="theme-on"><span><span></span></span><?php echo $text_yes; ?></label>
              <input type="radio" name="filter_theme" value="0" id="theme-off" class="radio" checked />
              <label for="theme-off"><span><span></span></span><?php echo $text_no; ?></label>
            <?php } ?></td>
          </tr>
        </tbody>
        <tbody id="header-1" class="module-header">
          <tr style="background:#FCFCFC;">
            ...
            <td><?php foreach ($languages as $language) { ?>
              <input type="text" name="filter_title<?php echo $language['language_id']; ?>" id="filter_title<?php echo $language['language_id']; ?>" size="30" value="<?php echo ${'filter_title' . $language['language_id']}; ?>" />
          ...
          <tr style="background:#FCFCFC;">
            ...
            <td><select name="filter_header_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $filter_header_color) { ?>
          ...
          <tr style="background:#FCFCFC;">
            ...
            <td><select name="filter_header_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $filter_header_shape) { ?>
        ...
        <tbody>
          <tr>
            ...
            <td><select name="filter_content_color">
              <?php foreach ($skins as $skin) { ?>
                <?php if ($skin['skin'] == $filter_content_color) { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;" selected="selected"><?php echo $skin['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $skin['skin']; ?>" style="background-color:<?php echo $skin['color']; ?>; padding:2px 4px;"><?php echo $skin['title']; ?></option>
                <?php } ?>
          ...
          <tr>
            ...
            <td><select name="filter_content_shape">
              <?php foreach ($shapes as $shape) { ?>
                <?php if ($shape['shape'] == $filter_content_shape) { ?>
                  <option value="<?php echo $shape['shape']; ?>" selected="selected"><?php echo $shape['title']; ?></option>
                <?php } else { ?>
                  <option value="<?php echo $shape['shape']; ?>"><?php echo $shape['title']; ?></option>
                <?php } ?>
        ...
        <?php foreach ($modules as $module) { ?>
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              ...
              <td class="center">
                <input type="text" name="filter_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" size="3" />
              </td>
              <td class="center">
...
$('input[name=\'filter_theme\']').bind('change', function() {
	$('.module-header').hide();
	$('#header-' + this.value).show();
});

$('input[name=\'filter_theme\']:checked').trigger('change');
//--></script>
...
function addModule() {
	...
	html += '    </select></td>';
	html += '    <td class="center"><input type="text" name="filter_module[' + module_row + '][sort_order]" value="" size="3" /></td>';
