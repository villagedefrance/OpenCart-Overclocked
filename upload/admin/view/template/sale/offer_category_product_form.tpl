      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <?php } else { ?>
          <tr>
            ...
            <td><?php if ($error_category) { ?>
              <input type="text" name="category_one_name" value="<?php echo $category_one_name; ?>" size="30" class="input-error" />
              <input type="hidden" name="category_one" value="<?php echo $category_one; ?>" />
              <span class="error"><?php echo $error_category; ?></span>
            <?php } else { ?>
              <input type="text" name="category_one_name" value="<?php echo $category_one_name; ?>" size="30" />
              <input type="hidden" name="category_one" value="<?php echo $category_one; ?>" />
            <?php } ?>
            </td>
          </tr>
...
$('input[name=\'category_one_name\']').autocomplete({
	delay: 10,
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=sale/offer_category_product/autocompleteCat&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request.term),
			dataType: 'json',
	...
	select: function(event, ui) {
		$('input[name=\'category_one_name\']').attr('value', ui.item.label);
		$('input[name=\'category_one\']').attr('value', ui.item.value);
...
$('input[name=\'product_two_name\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=sale/offer_category_product/autocompletePro&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {
				json.unshift({
					'product_id': 0,
