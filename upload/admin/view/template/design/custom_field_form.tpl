	...
	<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
      ...
      <table id="custom-field-value" class="list">
        ...
        <tfoot>
          <tr>
            <td colspan="2"></td>
            <td class="center"><a onclick="addCustomFieldValue();" class="button"><?php echo $button_add_custom_field_value; ?></a></td>
          </tr>
        </tfoot>
      </table>
    </form>
...
function addCustomFieldValue() {
	...
	<?php foreach ($languages as $language) { ?>
	html += '<input type="text" name="custom_field_value[' + custom_field_value_row + '][custom_field_value_description][<?php echo $language['language_id']; ?>][name]" value="" /> <img src="view/image/flags/<?php echo $language['image']; ?>" alt="" title="<?php echo $language['name']; ?>" /><br />';
	<?php } ?>
	html += '    </td>';
