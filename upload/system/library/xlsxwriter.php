...
class XLSXWriter {
	...
	public function writeToFile($filename) {
		...
		if (file_exists($filename)) {
			if (is_writable($filename)) {
				// If the zip already exists, remove it
				@unlink($filename);
			} else {
	...
	public function writeSheetRow($sheet_name, array $row) {
		...
		if (empty($sheet->columns)) {
			$sheet->columns = array_fill($from = 0, $until = count($row), '0');
		}
	...
	protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index) {
		...
		if (!is_scalar($value) || $value === '') {
			...
			$file->write('<c r="' . $cell_name . '" s="' . $cell_format_index . '" t="n"><v>' . self::xmlspecialchars($value) . '</v></c>');
