...
class Stripe_Object implements ArrayAccess {
    /**
     * @var Stripe_Util_Set Attributes that should not be sent to the API because they're not updatable (e.g. API key, ID).
     */
    public static $permanentAttributes;

    /**
     * @var Stripe_Util_Set Attributes that are nested but still updatable from the parent class's URL (e.g. metadata).
     */
    ...
    public function __set($k, $v) {
        ...
        if (self::$nestedUpdatableAttributes->includes($k) && isset($this->$k) && is_array($v)) {
            ...
            // may want to clear from $_transientValues (Won't be user-visible).
            $this->_values[$k] = $v;
        }
