    ...
    protected function adjustPageBreaks(PHPExcel_Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows) {
        ...
        foreach ($aBreaks as $key => $value) {
            if (self::cellAddressInDeleteRange($key, $beforeRow, $pNumRows, $beforeColumnIndex, $pNumCols)) {
                // If we're deleting, then clear any defined breaks that are within the range of rows/columns that we're deleting
                $pSheet->setBreak($key, PHPExcel_Worksheet::BREAK_NONE);
            } else {
                // Otherwise update any affected breaks by inserting a new break at the appropriate point and removing the old affected break
    ...
    public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExcel_Worksheet $pSheet = null) {
        ...
        while ($cellID = array_pop($aCellCollection)) {
            ...
            if (($cellIndex >= $beforeColumnIndex) && ($cell->getRow() >= $beforeRow)) {
                ...
                /* We don't need to update styles for rows/columns before our insertion position, but we do still need to adjust any formulae    in those cells */
                if ($cell->getDataType() == PHPExcel_Cell_DataType::TYPE_FORMULA) {
                    // Formula should be adjusted
            ...
            if ($pNumCols != 0) {
                ...
                if (count($autoFilterColumns) > 0) {
                    ...
                    if ($columnIndex <= $rangeEnd[0]) {
                        if ($pNumCols < 0) {
                            // If we're actually deleting any columns that fall within the autofilter range, then we delete any rules for those columns
