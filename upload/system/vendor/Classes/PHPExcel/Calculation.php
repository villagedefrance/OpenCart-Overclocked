...
class PHPExcel_Calculation {
    ...
    private static $controlFunctions = array(
        'MKMATRIX' => array('argumentCount' => '*', 'functionCall' => 'self::mkMatrix')
    );
    ...
    private static function translateFormula($from, $to, $formula, $fromSeparator, $toSeparator) {
        ...
        if (self::$localeLanguage !== 'en_us') {
            ...
            if (strpos($formula, '"') !== false) {
                // So instead we skip replacing in any quoted strings by only replacing in every other array element after we've exploded the formula
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null) {
        ...
        // Basic validation that this is indeed a formula. We simply return the cell value if not
        $formula = trim($formula);
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null) {
		...
		// We use this test in syntax-checking the expression to determine when a - is a negation or + is a positive operator rather than an operation
        $expectingOperator = false;
		// We use this test in syntax-checking the expression to determine whether an operand should be null in a function call
        $expectingOperand = false;

        // The guts of the lexical parser. Loop through the formula extracting each operator and operand in turn
        while (true) {
