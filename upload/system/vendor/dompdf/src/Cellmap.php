...
class Cellmap {
    ...
    protected function _resolve_border($i, $j, $h_v, $border_spec) {
        ...
        if (($n_style === "hidden" || $n_width > $o_width || $o_style === "none") || ($o_width == $n_width && in_array($n_style, self::$_BORDER_STYLE_SCORE) && self::$_BORDER_STYLE_SCORE[$n_style] > self::$_BORDER_STYLE_SCORE[$o_style])) {
            $border = $border_spec;
        }
    ...
    public function add_frame(Frame $frame) {
        ...
        if (!$this->_columns_locked) {
            ...
            for ($cs = 0; $cs < $colspan; $cs++) {
                ...
                // requested percentage or absolute values with the existing widths and adjust accordingly.
    ...
    public function assign_x_positions() {
        // Pre-condition: widths must be resolved and assigned to columns and column[0]["x"] must be set.
        if ($this->_columns_locked) {
            return;
        }
    ...
    public function assign_frame_heights() {
        // Pre-condition: widths and heights of each column & row must be calculated
