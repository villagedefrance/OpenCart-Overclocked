...
class Page_Cache {
  ...
  static function init() {
    if ( is_null(self::$__connection) ) {
      $con_str = "host=" . DB_HOST . " dbname=" . self::DB_NAME . " user=" . self::DB_USER . " password=" . self::DB_PASS;
  ...
  static function store_page($id, $page_num, $data) {
    $where = "WHERE id='" . pg_escape_string($id) . "' AND page_num=" . pg_escape_string($page_num);
    ...
    if ( $row ) {
      ...
      self::__query("INSERT INTO page_cache (id, page_num, data) VALUES ('" . pg_escape_string($id) . "', " . pg_escape_string($page_num) . ", '" . pg_escape_string($data) . "')");
    }
  }
  ...
  static function store_fonts($id, $fonts) {
    ...
    foreach (array_keys($fonts) as $font) {
      self::__query("INSERT INTO page_fonts (id, font_name) VALUES ('" . pg_escape_string($id) . "', '" . pg_escape_string($font) . "')");
	}
...
//   }
  ...
  static function get_page_timestamp($id, $page_num) {
    $res = self::__query("SELECT timestamp FROM page_cache WHERE id='" . pg_escape_string($id) . "' AND page_num=". pg_escape_string($page_num));
