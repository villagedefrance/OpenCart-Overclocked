	...
	public function listWorksheetInfo($pFilename) {
		...
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				...
				$xmlWorkbook = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					$xmlStyles = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());
					$numFmts = null;
									...
									foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
										...
										if ($filterColumn->filters) {
											...
											foreach ($filters->dateGroupItem as $dateGroupItem) {
												$column->createRule()->setRule(null,	array(
										...
										if ($filterColumn->customFilters) {
											...
											//	Custom filters can an AND or an OR join; and there should only ever be one or two entries
						...
						if ($xmlWorkbook->definedNames) {
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
									// Local defined name. Switch on type
									switch ((string)$definedName['name']) {
										case '_xlnm._FilterDatabase':
										case '_xlnm.Print_Titles':
										case '_xlnm.Print_Area':
